...
int main()
{
    ...
    if(Fptr!=NULL)
    {
            ...
            for (i=0; i<NRows1; i++) Matrices.Matrix1[i] =(int*)malloc(NColumns1*sizeof(int));
           ...
           fscanf(Fptr,"%d %d",&NRows2,&NColumns2);
            ...
            for (i=0; i<NRows2; i++) Matrices.Matrix2[i] =(int*)malloc(NColumns2*sizeof(int));
    ...
    for (i=0; i<NRows1; i++) Matrices.FMatrix[i] =(int*)malloc(NColumns2*sizeof(int));

    for(i=0;i<NRows1;i++) for(j=0;j<NColumns2;j++)  Matrices.FMatrix[i][j]=0; //Intializing ResultMatrix
